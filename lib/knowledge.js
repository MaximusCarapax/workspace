 ...
 */
function add({ title, summary, sourceType = 'manual', sourceUrl = null, sourceSession = null, tags = [], entities = [], confidence = 1.0, expiresAt = null }) {
  const stmt = db.prepare(`
    INSERT INTO knowledge_cache (
      title, summary, source_type, source_url, source_session,
      topic_tags, entities, confidence, expires_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
 ...
 */
function update(id, { summary, tags, confidence, expiresAt, verified }) {
