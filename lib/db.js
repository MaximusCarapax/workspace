...
function createPipeline({ projectId, parentId, title, description, priority }) {
  const stmt = db.prepare(`
    INSERT INTO pipeline (project_id, parent_id, title, description, priority)
    VALUES (?, ?, ?, ?, ?)
  `);
  return stmt.run(projectId || null, parentId || null, title, description || null, priority || 2).lastInsertRowid;
}
...
function listPipeline({ projectId, parentId, stage, limit } = {}) {
