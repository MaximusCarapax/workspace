...
function runMigrations() {
  ...
  // Migration: Add knowledge cache tables
  try {
    ...
    if (!knowledgeCacheExists) {
      ...
      try {
        ...
        console.warn('Could not create knowledge cache triggers:', triggerError.message);
      }
      
      // Create indexes
      db.exec(`CREATE INDEX IF NOT EXISTS idx_knowledge_source ON knowledge_cache(source_type)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_knowledge_created ON knowledge_cache(created_at)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_knowledge_verified ON knowledge_cache(verified)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_knowledge_expires ON knowledge_cache(expires_at)`);
