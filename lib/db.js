...
function createPipeline({ projectId, parentId, title, description, priority, type }) {
  const itemType = type || 'feature';
  const defaultStage = getDefaultStage(itemType);
  
  const stmt = db.prepare(`
    INSERT INTO pipeline (project_id, parent_id, title, description, priority, type, stage)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  return stmt.run(projectId || null, parentId || null, title, description || null, priority || 2, itemType, defaultStage).lastInsertRowid;
}
